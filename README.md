...
## Фидбек от HR и исправления

**Полученный фидбек:**
- неатомарные операции с БД
- конструктор запросов с помощью встроенных методов работы со строками
- в подсчете суммы не учитывается количество месяцев подписки
- не обработан случай изменения несуществующей подписки
- сравнение ошибок через `==`

**Что было сделано:**

1. **Неатомарные операции с БД** — метод `List` теперь использует транзакцию `RunInTx` для атомарного выполнения COUNT и SELECT. Это гарантирует согласованность данных между подсчётом общего количества и получением списка подписок.

2. **Конструктор запросов через строки** — миграция на **Bun ORM** (uptrace/bun) с родной интеграцией pgx. Вместо небезопасного `fmt.Sprintf(" AND user_id = $%d", argIdx)` используется типизированный query builder Bun с параметризованными запросами: `.Where("user_id = ?", input.UserID)`.

3. **Подсчет суммы без учёта месяцев** — метод `TotalCost` теперь:
   - Выбирает все подписки, пересекающие запрашиваемый период
   - Вычисляет количество пересекающихся месяцев через функцию `overlappingMonths()`
   - Умножает месячную цену на количество месяцев пересечения и суммирует
   - Добавлены юнит-тесты для логики расчёта (`repository_test.go`)

4. **Изменение несуществующей подписки** — метод `Update` проверяет `RowsAffected()`. Если запись не найдена (0 строк затронуто), возвращает `domain.ErrSubscriptionNotFound`. В `handler.go` это теперь корректно возвращает HTTP 404.

5. **Сравнение ошибок через `==`** — заменено на `errors.Is(err, domain.ErrSubscriptionNotFound)` и `errors.Is(err, sql.ErrNoRows)` для правильной обработки обёрнутых ошибок.
